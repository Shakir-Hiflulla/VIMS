        ...
        <div class="agent_div">
            ...
            <?php
                ...
                if ($result->num_rows > 0){
                    ...
                    {
                        $sql1 = "SELECT COUNT(*) AS count FROM policy WHERE user_id={$row['user_id']}";
                        $result1 = $con->query($sql1);
                        $rows=$result1->fetch_assoc();
                        $count=$rows['count'];
                        echo "<tr>";
                        echo '<td>'.$row['user_id'].'</td><td>'.$row['f_name']." ".$row['l_name'].'</td><td>'.$row['address'].'</td><td>'.$row['dob'].'</td><td>'.$row['email'].'</td><td>'.$count.'</td>';
                        if($_SESSION['staff']==1){
                            echo '<td>'.$row['username'].'</td>';
                        }    
                        echo '<td>
                        <form method="post" action="user.php">
                            <input type="hidden" name="dlt" value="'.$row['user_id'].'">
